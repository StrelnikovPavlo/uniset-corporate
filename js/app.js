 ...
 */
    ...
    (() => {
        ...
        function initSliders() {
            ...
            if (document.querySelector(".info-company__slider")) {
                new Swiper(".info-company__slider", {
                    modules: [ Pagination, Navigation ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 1.2,
                    ...
                    speed: 400,
                    pagination: {
                        el: ".info-company__pagging",
                        clickable: true
                    },
                    navigation: {
                        prevEl: ".info-company__nav_prev",
                        nextEl: ".info-company__nav_next"
                    },
                    breakpoints: {
                        480: {
                            slidesPerView: 2,
                    ...
                    on: {
                        init: function() {
                            if (this.slides.length <= 4) {
                                const pagination = document.querySelector(".info-company__pagging");
...
***************************************************************************** */
        ...
        showMoreElements.forEach((showMoreElement => {
            ...
            const maxParagraphsToShow = parseInt(showMoreElement.getAttribute("maxParagraphsToShow")) || 3;
            const childElements = contentElement.children;
            if (childElements.length <= maxParagraphsToShow) buttonElement.style.display = "none";
            for (let i = maxParagraphsToShow; i < childElements.length; i++) childElements[i].style.display = "none";
            buttonElement.addEventListener("click", (function() {
                const isContentHidden = childElements[maxParagraphsToShow].style.display === "none";
                for (let i = maxParagraphsToShow; i < childElements.length; i++) childElements[i].style.display = isContentHidden ? "block" : "none";
                const buttonText = isContentHidden ? "Показати менше" : buttonTextContent;
                buttonElement.querySelector("span").textContent = buttonText;
                if (!isContentHidden && childElements.length <= maxParagraphsToShow) buttonElement.style.display = "none";
                showMoreElement.classList.toggle("_showmore-active", isContentHidden);
            }));
        }));
        ...
        function createLazyLinePainter(selector, delay = 0) {
            ...
            if (element) {
                let myAnimation = new LazyLinePainter(element, {
                    ease: "easeLinear",
                    strokeWidth: 1,
                    strokeOpacity: 1,
                    strokeColor: "#DADADA",
        ...
        const lines = [ {
            selector: ".lazy-line-painter-1",
            delay: 0
        }, {
            selector: ".lazy-line-painter-2",
            delay: 500
        }, {
            selector: ".lazy-line-painter-3",
            delay: 1e3
        }, {
            selector: ".lazy-line-painter-4",
            delay: 1e3
        }, {
            selector: ".lazy-line-painter-5",
            delay: 1e3
        }, {
            selector: ".lazy-line-painter-6",
            delay: 0
        } ];
        lines.forEach((line => createLazyLinePainter(line.selector, line.delay)));
        const targetElement = document.querySelector(".project-solutions__dots");
        if (targetElement) {
            function paintLazyLine(element, delay) {
                if (element) {
                    let myAnimation = new LazyLinePainter(element, {
                        ease: "easeLinear",
                        strokeWidth: 1,
            ...
            const callback = (entries, observer) => {
                entries.forEach((entry => {
                    if (entry.isIntersecting) {
                        paintLazyLine(document.querySelector(".lazy-line-painter-1"), 0);
                        paintLazyLine(document.querySelector(".lazy-line-painter-2"), 500);
                        paintLazyLine(document.querySelector(".lazy-line-painter-3"), 1e3);
                        paintLazyLine(document.querySelector(".lazy-line-painter-4"), 1e3);
                        paintLazyLine(document.querySelector(".lazy-line-painter-5"), 1e3);
                        paintLazyLine(document.querySelector(".lazy-line-painter-6"), 0);
                        observer.disconnect();
                    }
                }));
            };
